...
entropy_data1 <- speech_acts %>%
  group_by(child_id, speaker, age_months) %>%
...
filtered_transition_probs <- filtered_transition_probs %>%
  group_by(act_code) %>%
  mutate(conditional_probability = probability / sum(probability)) %>%
  ungroup()

# Calculate log-transformed conditional probabilities
filtered_transition_probs$log_probability <- log(filtered_transition_probs$conditional_probability)

# Create a custom order for act_code
custom_order <- unique(filtered_transition_probs$act_code)

# Set the levels of act_code to maintain consistency
filtered_transition_probs$act_code <- factor(filtered_transition_probs$act_code, levels = custom_order)

# Create a transition probability matrix
transition_matrix_filtered <- dcast(filtered_transition_probs, act_code ~ next_act, value.var = "log_probability", fill = 0)

transition_matrix_filtered <- transition_matrix_filtered %>%
  select(-AD)

# Reshape the matrix for plotting
transition_matrix_melted <- melt(transition_matrix_filtered, id.vars = "act_code")

# Plot the heatmap
ggplot(data = transition_matrix_melted, aes(x = act_code, y = variable, fill = -value)) +
  geom_tile(color = "lightgrey",
            lwd = 1.5,
            linetype = 1) +
  coord_fixed() +
  scale_fill_gradient(low = "antiquewhite", high = "darkblue") +
  labs(
    x = "Next Act",
    y = "Current Act",
    fill = "Log Prob. (closer to 0 is higher prob)"
  ) +
  labs(title = "Conditional Prob of Next CA Given Current One") +
