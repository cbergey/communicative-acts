...
babble_data <- clean_speech_acts %>%
  filter(utterance %in% babble_list)

babble_data <- babble_data %>%
  mutate(
    ...
    speaker_role = if_else(speaker == "CHI", "Child", "Parent"),
    speaker_role = factor(speaker_role, levels = c("Child", "Parent"))) %>%
  mutate(utterance = ifelse(utterance %in% babble_list, "babble", utterance))

# Calculate the total n for the meaning column in clean_speech_acts
total_n_meaning <- summarise(clean_speech_acts, total_n_meaning = n())

# Calculate the babble_summary
babble_summary <- babble_data %>%
  group_by(age_bin, speaker_role, utterance) %>%
  count() %>%
  left_join(total_n_meaning, by = character()) %>%
  mutate(prob = n / total_n_meaning) %>%
  ungroup() %>%
  select(-total_n_meaning)

ggplot(babble_summary, aes(x = age_bin, y = prob, group = utterance, color = as.factor(utterance))) +
  geom_line() +
  facet_wrap(~speaker_role) +
  theme_few(base_size = 10) +
  theme(legend.position = "none") +
  scale_x_continuous(breaks = c(12, 24, 36, 48, 60)) +
  labs(title = "Babbling as a Function of Time", x = "Age (months)", y = "Babble Proportion")
```